...
def save_ER_to_file(ERmap):  # 将ER邻边连接随机网格写入txt文档
    file = open(path + 'edges.csv', 'a+')
    for i in range(len(ERmap)):  # 打开文档，若无将自动创建
        a = ERmap[i]
        for j in range(len(ERmap)):
            if a[j] == 1:  # 先取第一行，再取各列
                file.write(str(i) + ', ' + str(j) + ', ' + '1\n')# TODO 边权默认为1
                file.write(str(j) + ', ' + str(i) + ', ' + '1\n')# TODO 边权默认为1
    file.close()

def save_ER_new_to_file(ERmap,degrees, days: int):  # 节点状态
    file = open(path + 'nodes' + str(days) + '.csv', 'a+')
...
def calculateDegreeDistribution(ERmap):
    ...
    plt.savefig(path + '度分布图.pdf')
    plt.show()  # 展示图片
    print('算法正常运行则概率之和应为1 当前概率之和=\t' + str(identify))
    # 用于测试算法是否正确
    f = open(path + '度分布.txt', 'w+')
    # 将度分布写入文件名为度分布.txt中
    # 若磁盘中无此文件将自动新建
    for i in range(len(ERmap)):
...
def epedemic_Simulation(N, p, S_to_E, E_to_I, to_R, t):
    ...
    #写入csv header
    file = open(path + 'nodes' + str(t) + '.csv', 'w+')
    file.write('state, days\n')
    file = open(path + 'edges.csv', 'w+')
    file.write('src, dst, weight\n')
    file.close
    ...
    for a in range(Rp):  # 重复实验次数Rp次，利用for套内循环
        ...
        for i in range(t):
            if i != 0:
                ...
                save_ER_new_to_file(ERmap, degrees, i)
            else:
                Ss.append(np.where(np.array(degrees) == 1, 1, 0).sum())
                Ee.append(np.where(np.array(degrees) == 2, 1, 0).sum())
                Ii.append(np.where(np.array(degrees) == 3, 1, 0).sum())
