...
def create_BA(nodes: int, add_edges: int, file_path: str, random_seed=None):
    BA = nx.random_graphs.barabasi_albert_graph(nodes, add_edges, seed = random_seed)
...
def calculateDegreeDistribution(file_path, BAmap):
    ...
    p_degree = np.zeros((len(BAmap)), dtype=float)
    # statistic下标为度值
    # (1)先计数该度值的量
    # (2)再除以总节点数N得到比例
    degree = np.zeros(len(BAmap), dtype=int)
    # degree用于存放各个节点的度之和
    ...
    # 纵坐标标注——P
    plt.plot(list(range(len(BAmap))), list(p_degree), '-*', markersize=15, label='度', color="#ff9c00")
